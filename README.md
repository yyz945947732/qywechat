...
| content               | String   | 是       | 文本内容，最长不超过 2048 个字节，必须是 utf8 编码                                                                           |
| mentioned_list        | String[] | 否       | userid 的列表，提醒群中的指定成员(@某个成员)，@all 表示提醒所有人，如果开发者获取不到 userid，可以使用 mentioned_mobile_list |
| mentioned_mobile_list | String[] | 否       | 手机号列表，提醒手机号对应的群成员(@某个成员)，@all 表示提醒所有人                                                           |
...
| ------- | ------ | -------- | ------------------------------------------------------- |
| url     | String | 是       | 机器人的 webhook 地址                                   |
| content | String | 是       | markdown 内容，最长不超过 4096 个字节，必须是 utf8 编码 |
...
| ------ | ------ | -------- | ---------------------------------- |
| url    | String | 是       | 机器人的 webhook 地址              |
| base64 | String | 是       | 图片内容的 base64 编码             |
| md5    | String | 是       | 图片内容（base64 编码前）的 md5 值 |
...
| articles.title       | String   | 是       | 标题，不超过 128 个字节，超过会自动截断                                           |
| articles.description | String   | 否       | 描述，不超过 512 个字节，超过会自动截断                                           |
| articles.url         | String   | 是       | 点击后跳转的链接                                                                  |
| articles.picurl      | String   | 否       | 图文消息的图片链接，支持 JPG、PNG 格式，较好的效果为大图 1068✖️455，小图 150✖️150 |
...
| -------- | ------ | -------- | ----------------------------------- |
| url      | String | 是       | 机器人的 webhook 地址               |
| media_id | String | 是       | 文件 id，通过下文的文件上传接口获取 |
...
| -------- | ------ | -------- | --------------------------------------- |
| url      | String | 是       | 机器人的 webhook 地址                   |
| media_id | String | 是       | 语音文件 id，通过下文的文件上传接口获取 |
